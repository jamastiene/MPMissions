...
if(_free_magazine_slots < 1) exitWith { 
	...
	cutText ["Il n'y a plus de place dans votre inventaire.", "PLAIN DOWN"];
	DZE_ActionInProgress = false;
};
...
for "_x" from 1 to _total_trades do {
	...
	if(_total_trades == 1) then { 
		cutText [format["Transaction en cours restez immobile.",_tradeCounter,_total_trades] , "PLAIN DOWN"];
	} else {
		cutText [format["Transaction en cours restez immobile %1 of %2.",_tradeCounter,_total_trades] , "PLAIN DOWN"];
	};
